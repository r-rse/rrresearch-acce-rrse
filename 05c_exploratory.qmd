...
We can see that there are **very few entries for `liana` and also a whole bunch of `NA`s in `growth_form`.** So let's **remove any such rows** from our data by using `dplyr::filter()`. 
